...
public class MediaPlaybackService extends Service {
  ...
  String[]                    mCursorCols                   = new String[] {
      ...
      MediaStore.Audio.Media.ARTIST_ID, MediaStore.Audio.Media.IS_PODCAST, // index must match PODCASTCOLIDX below
  ...
  // MediaAppWidgetProvider.getInstance();
  ...
  private Handler             mMediaplayerHandler           = new MediaMessageHandler(this);

  private static class MediaMessageHandler extends Handler {
    private final WeakReference<MediaPlaybackService> service;

    public MediaMessageHandler(MediaPlaybackService service) {
      this.service = new WeakReference<MediaPlaybackService>(service);
    }
    ...
    public void handleMessage(Message msg) {
      ...
      case FADEDOWN:
        ...
        if (mCurrentVolume > .2f) {
          getService().mMediaplayerHandler.sendEmptyMessageDelayed(FADEDOWN, 10);
        } else {
          mCurrentVolume = .2f;
        }
        getService().mPlayer.setVolume(mCurrentVolume);
        break;
      case FADEUP:
        mCurrentVolume += .01f;
        if (mCurrentVolume < 1.0f) {
          getService().mMediaplayerHandler.sendEmptyMessageDelayed(FADEUP, 10);
        } else {
          mCurrentVolume = 1.0f;
        }
        getService().mPlayer.setVolume(mCurrentVolume);
        break;
      case SERVER_DIED:
        if (getService().mIsSupposedToBePlaying) {
          getService().gotoNext(true);
        } else {
          // the server died when we were idle, so just
          // reopen the same song (it will start again
          // from the beginning though when the user
          // restarts)
          getService().openCurrentAndNext();
        }
        break;
      case TRACK_WENT_TO_NEXT:
        getService().mPlayPos = getService().mNextPlayPos;
        if (getService().mCursor != null) {
          getService().mCursor.close();
          getService().mCursor = null;
        }
        getService().mCursor = getService().getCursorForId(getService().mPlayList[getService().mPlayPos]);
        getService().notifyChange(META_CHANGED);
        getService().updateNotification();
        getService().setNextTrack();
        break;
      case TRACK_ENDED:
        if (getService().mRepeatMode == REPEAT_CURRENT) {
          getService().seek(0);
          getService().play();
        } else {
          getService().gotoNext(false);
        }
        break;
      case RELEASE_WAKELOCK:
        getService().mWakeLock.release();
        break;
      ...
      case FOCUSCHANGE:
        ...
        case AudioManager.AUDIOFOCUS_LOSS:
          ...
          if (getService().isPlaying()) {
            getService().mPausedByTransientLossOfFocus = false;
          }
          getService().pause();
          break;
        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
          getService().mMediaplayerHandler.removeMessages(FADEUP);
          getService().mMediaplayerHandler.sendEmptyMessage(FADEDOWN);
          break;
        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
          Log.v(LOGTAG, "AudioFocus: received AUDIOFOCUS_LOSS_TRANSIENT");
          if (getService().isPlaying()) {
            getService().mPausedByTransientLossOfFocus = true;
          }
          getService().pause();
          break;
        case AudioManager.AUDIOFOCUS_GAIN:
          Log.v(LOGTAG, "AudioFocus: received AUDIOFOCUS_GAIN");
          if (!getService().isPlaying() && getService().mPausedByTransientLossOfFocus) {
            getService().mPausedByTransientLossOfFocus = false;
            mCurrentVolume = 0f;
            getService().mPlayer.setVolume(mCurrentVolume);
            getService().play(); // also queues a fade-in
          } else {
            getService().mMediaplayerHandler.removeMessages(FADEDOWN);
            getService().mMediaplayerHandler.sendEmptyMessage(FADEUP);
          }
          break;
        default:
  ...
  private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
        ...
        seek(0);
        // } else if
        // (MediaAppWidgetProvider.CMDAPPWIDGETUPDATE.equals(cmd))
        // {
  ...
  public void onCreate() {
    ...
    /*
     * reloadQueue(); notifyChange(QUEUE_CHANGED); notifyChange(META_CHANGED);
     */
  ...
  private void reloadQueue() {
    ...
    if (qlen > 1) {
      ...
      Cursor crsr = MusicUtils.query(this, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, new String[] { "_id" }, "_id="
          + mPlayList[mPlayPos], null, null);
  ...
  public boolean onUnbind(Intent intent) {
    ...
    if (isPlaying() || mPausedByTransientLossOfFocus) {
      ...
      // an in-progress action requesting audio focus ends, so don't stop the
      // service now.
      return true;
    }
  ...
  private Handler             mDelayedStopHandler           = new DelayedStopHandler(this);

  private static class DelayedStopHandler extends Handler {
    private final WeakReference<MediaPlaybackService> service;

    public DelayedStopHandler(MediaPlaybackService service) {
      this.service = new WeakReference<MediaPlaybackService>(service);
    }
    MediaPlaybackService getService() {
      return this.service.get();
    ...
    public void handleMessage(Message msg) {
      ...
      if (this.getService().isPlaying() || this.getService().mPausedByTransientLossOfFocus
          || this.getService().mServiceInUse || this.getService().mMediaplayerHandler.hasMessages(TRACK_ENDED)) { return; }
      // save the queue again, because it might have changed since the user exited the music app
      // (because of party-shuffle or because the play-position changed)
      this.getService().saveQueue(true);
      this.getService().stopSelf(this.getService().mServiceStartId);
    }
  };
   ...
   * intent will call closeExternalStorageFiles() if the external media is going
   * to be ejected, so applications can clean up any files they have open.
   */
  ...
  /**
   * Notify the change-receivers that something has changed. The intent that is
   * sent contains the following data for the currently playing track: "id" -
   ...
   * playback queue has changed, that playback has stopped because the last file
   * in the list has been played, or that the play-state changed
   * (paused/resumed).
   */
  private void notifyChange(String what) {
    ...
    // TODO: mAppWidgetProvider.notifyChange(this, what);
  }
   ...
   * playback at the specified position in the list, or a random position if the
  ...
  private Cursor getCursorForId(long lid) {
    ...
    Cursor c = getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, mCursorCols, "_id=" + id, null,
        null);
    c.moveToFirst();
    return c;
  }
    ...
    synchronized (this) {
      ...
      while (!open(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/" + mCursor.getLong(IDCOLIDX))) {
        if (mOpenFailedCounter++ < 10 && mPlayListLen > 1) {
          ...
          if (!mQuietMode) {
            // TODO: Toast.makeText(this, R.string.playback_failed,
  ...
  /*
   * Desired behavior for prev/next/shuffle: - NEXT will move to the next track
   * in the list when not shuffling, and to a track randomly picked from the
   * not-yet-played tracks when shuffling. If all tracks have already been
   * played, pick from the full set, but avoid picking the previously played
   * track if possible. - when shuffling, PREV will go to the previously played
   * track. Hitting PREV again will go to the track played before that, etc.
   * When the start of the history has been reached, PREV is a no-op. When not
   * shuffling, PREV will go to the sequentially previous track (the difference
   * with the shuffle-case is mainly that when not shuffling, the user can back
   * up to tracks that are not in the history). Example: When playing an album
   * with 10 tracks from the start, and enabling shuffle while playing track 5,
   * the remaining tracks (6-10) will be shuffled, e.g. the final play order
   * might be 1-2-3-4-5-8-10-6-9-7. When hitting 'prev' 8 times while playing
   * track 7 in this example, the user will go to tracks 9-6-10-8-5-4-3-2. If
   * the user then hits 'next', a random track will be picked again. If at any
   * time user disables shuffling the next/previous track will be picked in
    ...
    if (mRepeatMode == REPEAT_CURRENT) {
      ...
      if (mPlayPos >= mPlayListLen - 1) {
        ...
        if (mRepeatMode == REPEAT_NONE && !force) {
          // all done
          return -1;
        } else if (mRepeatMode == REPEAT_ALL || force) { return 0; }
        return -1;
      } else {
        return mPlayPos + 1;
      }
  ...
  private boolean wasRecentlyUsed(int idx, int lookbacksize) {
    ...
    if (lookbacksize == 0) { return false; }
  ...
  /**
   * Removes the range of tracks specified from the play list. If a file within
   * the range is the file currently being played, playback will move to the
   * next file after the range.
   * 
   * @param first
   ...
   * ensure that the Service can be GCd even when the system process still has a
   * remote reference to the stub.
   */
